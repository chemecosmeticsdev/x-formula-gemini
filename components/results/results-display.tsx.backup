  ...
  const generateFormula = async (data: FormData) => {
    try {
      ...
      try {
        ...
        // For now, let's use a temporary approach and call a dedicated image generation function
        await generateProductMockupImage(formulaData, data, GEMINI_API_KEY);

        async function generateProductMockupImage(formula: any, userInput: any, apiKey: string) {
          try {
            const imagePrompt = `Professional product photography: "${formula.name}" - ${formula.type}. 
            
            Details: ${formula.description}
            User request: ${userInput.productDescription}
            
            Style: High-end cosmetics photography, clean background, professional lighting, luxury packaging, ${formula.type.toLowerCase()} container, commercial product shot.`;

            console.log('📝 Generating image with prompt...');
            
            const response = await fetch(`https://i.ytimg.com/vi/f0igz-skLUY/hq720.jpg?sqp=-oaymwEhCK4FEIIDSFryq4qpAxMIARUAAAAAGAElAADIQj0AgKJD&rs=AOn4CLBecdvcsg_ukQwUJ9ZApGTGC_oyIQ {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                prompt: imagePrompt,
                imageGenerationConfig: {
                  aspectRatio: "4:3",
                  negativePrompt: "blurry, low quality, text, amateur, cartoon"
                }
              })
            });
            ...
            if (response.ok) {
              const result = await response.json();
              console.log('📦 Image API result keys:', Object.keys(result));
              console.log('🖼️ Full image response:', JSON.stringify(result, null, 2));
              
              const imageData = result.candidates?.[0]?.image?.bytesBase64Jpeg;
              if (imageData) {
                formula.mockup_image = `data:image/jpeg;base64,${imageData}`;
                console.log('✅ Successfully generated AI product mockup!');
                return;
              }
            } else {
              const errorText = await response.text();
          ...
          formula.mockup_image = "/images/placeholder-mockup.jpg";
        }
